...
// Alarm selection is asset-based. We used to support platform ringtones
// via a MethodChannel; that approach was removed in favor of bundled
// assets. The code now lists audio files from the `AssetManifest.json`
// and plays the selected asset.
...
class SettingsNotifier extends StateNotifier<String?> {
  ...
  static const _kAlarmKey = 'selected_alarm_asset';
  Future<void> _load() async {
    final prefs = await SharedPreferences.getInstance();
    state = prefs.getString(_kAlarmKey);
  }

  Future<void> setAlarmAsset(String? assetPath) async {
    final prefs = await SharedPreferences.getInstance();
    if (assetPath == null)
      await prefs.remove(_kAlarmKey);
    else
      await prefs.setString(_kAlarmKey, assetPath);
    state = assetPath;
  }
}
...
class RunnerNotifier extends StateNotifier<RunnerState> {
  ...
  Future<void> _playAlarm() async {
    ...
    final selectedAsset = ref.read(settingsProvider);
    if (selectedAsset != null && selectedAsset.isNotEmpty) {
      try {
        await _player!.play(AssetSource(selectedAsset));
        return;
      } catch (_) {}
    }

    // Try default bundled alarm asset
    try {
      await _player!.play(AssetSource('assets/alarm.wav'));
      return;
    } catch (_) {}

    // Fallback: write asset bytes to a temp file and play (older platforms)
    try {
      final bytes =
    ...
    // Last-resort: generate a simple sine WAV and play it
    try {
                ...
                if (assets.isEmpty) {
                  if (mounted)
                    ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('No alarm assets found')));
                  return;
                }
                    ...
                    builder: (ctx) => SimpleDialog(
                          ...
                          children: assets
                              .map((a) => SimpleDialogOption(
                                  ...
                                  child: Text(a.replaceFirst('assets/', ''))))
                              .toList(),
                        ));
                if (picked != null) {
                  await ref
                      .read(settingsProvider.notifier)
                      .setAlarmAsset(picked);
                  if (mounted)
                    ScaffoldMessenger.of(context).showSnackBar(
